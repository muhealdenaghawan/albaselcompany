  ...
  return (
    <DialogContext.Provider value={{ innerOpen, setInnerOpen }}>
      <DialogPrimitive.Root
        open={open ?? outerOpen}
        onOpenChange={onOpenChange ?? setOuterOpen}
      >
        {children}
      </DialogPrimitive.Root>
...
const OuterDialogOverlay = React.forwardRef<
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      "fixed inset-0 z-50 bg-black/80 custom-scrollbar backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 overflow-y-auto",
      className
    )}
...
OuterDialogOverlay.displayName = DialogPrimitive.Overlay.displayName;
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      "fixed inset-0 z-[60] bg-black/70 custom-scrollbar backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 overflow-y-auto",
      className
    )}
    style={{ display: "block", padding: "1rem 0" }}
    {...props}
...
InnerDialogOverlay.displayName = DialogPrimitive.Overlay.displayName;
...
>(({ className, children, ...props }, ref) => {
  ...
  return (
    <DialogPortal>
      <OuterDialogOverlay>
        <div className="fixed inset-0 z-50 flex items-start justify-center sm:items-start">
          <DialogPrimitive.Content
            ref={ref}
            className={cn(
              "fixed left-[50%] top-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] rounded-lg border bg-background p-6 shadow-lg duration-200 custom-scrollbar overflow-y-auto max-h-[90vh]",
...
>(
  (
    ...
    return (
      <DialogPortal>
        <InnerDialogOverlay className="flex items-start justify-center overflow-y-auto">
          <div className="fixed inset-0 z-[60]  flex items-start justify-center sm:items-start ">
            <DialogPrimitive.Content
              ref={ref}
            ...
            >
              ...
              <InnerDialogClose className="absolute end-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground">
                <X className="h-4 w-4" />
                <span className="sr-only">Close</span>
              </InnerDialogClose>
            </DialogPrimitive.Content>
