...
export function NavGroup({ title, items }: NavGroup) {
  console.log("🚀 ~ NavGroup ~ title:", title);
...
const SidebarMenuLink = ({ item, href }: { item: NavLink; href: string }) => {
  ...
  const role = auth.account?.user.role as (typeof RolesConstant)[keyof typeof RolesConstant];
  const showItem = role ? item.role.includes(role) : false;
  if (!showItem) return <></>;
  return (
    <SidebarMenuItem>
          ...
          <SidebarMenuSub>
            {item.items.map((subItem) => (
              <SidebarMenuSubItem key={subItem.title}>
                <SidebarMenuSubButton asChild isActive={checkIsActive(href, subItem as NavItem)}>
